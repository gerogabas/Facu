...
Create table resumenes_capitulo(
	elem_id ISBN REFERENCES elementos(elem_id),
...
create table libros (
elem_id ISBN PRIMARY KEY REFERENCES elementos(elem_id),
tipo_libro int REFERENCES tipo_libro(id_tipo_libro),
id_editorial int REFERENCES editorial(editorial_id),
...
tipo_periodo varchar(1) REFERENCES periodo_revista(tipo_periodo),
...
Create table prestamosDocentes(
	cod_prestamo serial primary key,
	elemento_id ISBN,
	nro_ejemplar int,
	cod_empleado varchar(1) references empleados(cod_empleado),
...
Create table autor_articulo(
	...
	Foreign key(elem_id, nro_articulo) references articulo_revista(elem_id, nro_articulo),
